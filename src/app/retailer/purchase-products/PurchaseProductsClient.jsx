...
const PurchaseProductsClient = () => {
  ...
  return (
    <div className="page-content-tile">
      ...
      {/* Modals and checkout UI copied from original page... kept as-is */}
      {showDraftModal && (
        <div className="modal-overlay" style={{ position: 'fixed', top:0, left:0, right:0, bottom:0, backgroundColor:'rgba(0,0,0,0.5)', display:'flex', justifyContent:'center', alignItems:'center', zIndex:1000 }}>
          <div className="modal-content" style={{ backgroundColor:'white', padding:'20px', borderRadius:'8px', width:'400px', maxWidth:'90%' }}>
            ...
            <div style={{display:'flex', gap:'10px', justifyContent:'flex-end'}}>
              <button className="enhanced-submit-btn" onClick={() => setShowDraftModal(false)} style={{backgroundColor:'#6c757d'}}>Cancel</button>
              <button className="enhanced-submit-btn" onClick={saveAsDraft}>Save Draft</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showCheckoutModal && (
        <div className="modal-overlay" style={{ position: 'fixed', top:0, left:0, right:0, bottom:0, backgroundColor:'rgba(0,0,0,0.5)', display:'flex', justifyContent:'center', alignItems:'center', zIndex:1000 }}>
          <div className="modal-content" style={{ backgroundColor:'white', padding:'20px', borderRadius:'8px', width:'500px', maxWidth:'90%', maxHeight:'80vh', overflowY:'auto' }}>
            ...
            <div style={{marginBottom:'20px'}}>
              ...
              <div style={{textAlign:'right', marginTop:'10px', fontWeight:'bold'}}>Total: ₹{getTotal()}</div>
            </div>

            {/* Delivery Address Form and contact fields (kept as in original) */}
            <div style={{marginBottom:'20px'}}>
              <h4>Delivery Address</h4>
              <div style={{display:'grid', gap:'10px'}}>
                <div>
                  <label htmlFor="deliveryAddress" style={{display:'block', marginBottom:'5px', fontWeight:'bold'}}>Address:</label>
                  <textarea id="deliveryAddress" value={checkoutData.deliveryAddress} onChange={(e) => setCheckoutData(prev => ({ ...prev, deliveryAddress: e.target.value }))} placeholder="Enter delivery address" rows="3" style={{ width:'100%', padding:'8px', border:'1px solid #ccc', borderRadius:'4px', fontSize:'14px', resize:'vertical' }} required />
                </div>
                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px'}}>
                  <div>
                    <label htmlFor="city" style={{display:'block', marginBottom:'5px', fontWeight:'bold'}}>City:</label>
                    <input type="text" id="city" value={checkoutData.city} onChange={(e) => setCheckoutData(prev => ({ ...prev, city: e.target.value }))} placeholder="City" style={{ width:'100%', padding:'8px', border:'1px solid #ccc', borderRadius:'4px', fontSize:'14px' }} required />
                  </div>
                  <div>
                    <label htmlFor="state" style={{display:'block', marginBottom:'5px', fontWeight:'bold'}}>State:</label>
                    <input type="text" id="state" value={checkoutData.state} onChange={(e) => setCheckoutData(prev => ({ ...prev, state: e.target.value }))} placeholder="State" style={{ width:'100%', padding:'8px', border:'1px solid #ccc', borderRadius:'4px', fontSize:'14px' }} required />
                  </div>
                </div>
                <div>
                  <label htmlFor="zipCode" style={{display:'block', marginBottom:'5px', fontWeight:'bold'}}>ZIP Code:</label>
                  <input type="text" id="zipCode" value={checkoutData.zipCode} onChange={(e) => setCheckoutData(prev => ({ ...prev, zipCode: e.target.value }))} placeholder="ZIP Code" style={{ width:'100%', padding:'8px', border:'1px solid #ccc', borderRadius:'4px', fontSize:'14px' }} required />
                </div>
              </div>
            </div>
            ...
            <div style={{marginBottom:'20px'}}>
              ...
              <div style={{display:'grid', gap:'10px'}}>
                <div>
                  ...
                  <input type="tel" id="contactNumber" value={checkoutData.contactNumber} onChange={(e) => setCheckoutData(prev => ({ ...prev, contactNumber: e.target.value }))} placeholder="Contact Number" style={{ width:'100%', padding:'8px', border:'1px solid #ccc', borderRadius:'4px', fontSize:'14px' }} required />
                </div>
                <div>
                  <label htmlFor="email" style={{display:'block', marginBottom:'5px', fontWeight:'bold'}}>Email:</label>
                  <input type="email" id="email" value={checkoutData.email} onChange={(e) => setCheckoutData(prev => ({ ...prev, email: e.target.value }))} placeholder="Email" style={{ width:'100%', padding:'8px', border:'1px solid #ccc', borderRadius:'4px', fontSize:'14px' }} required />
                </div>
              </div>
            </div>
            ...
            <div style={{marginBottom:'20px'}}>
              ...
              <div style={{padding:'10px', backgroundColor:'#f8f9fa', borderRadius:'4px', border:'1px solid #dee2e6'}}>
                ...
                <p style={{margin:'5px 0 0 0', fontSize:'14px', color:'#6c757d'}}>Pay when your order is delivered to your doorstep.</p>
              </div>
            </div>

            <div style={{display:'flex', gap:'10px', justifyContent:'flex-end'}}>
              <button className="enhanced-submit-btn" onClick={() => setShowCheckoutModal(false)} style={{backgroundColor:'#6c757d'}}>Cancel</button>
              <button className="enhanced-submit-btn" onClick={handleCheckout} disabled={isSubmitting} style={{backgroundColor: isSubmitting ? '#6c757d' : '#28a745'}}>{isSubmitting ? 'Placing Order...' : 'Place Order (COD)'}</button>
            </div>
          </div>
        </div>
      )}
...
export default PurchaseProductsClient;
